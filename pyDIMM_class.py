...
#TODO: Transpose the matrix `observe_data`, `delta`, `log_prob`, `cells` in Python and C.
...
class DIMM:
    ...
    def __init__(
        ...
        self.__lib.EM_with_1dArr.argtypes = [c_int, c_int, c_int, ctypes.POINTER(c_double), ctypes.POINTER(c_double), ctypes.POINTER(c_double), c_int, c_double, c_double, c_double, ctypes.POINTER(c_double), ctypes.POINTER(c_double)]
    ...
    def __init_alpha_pie(
        ...
        if (init_mode == 'LoadingFromFile'):
            ...
            cluster_index_test,cluster_index_test_index  = np.unique(cluster_cell, return_index= True)
            tmp_index = pd.DataFrame(cluster_index_test_index).sort_values(by = 0)
            cluster_cell_new = copy.deepcopy(cluster_cell)
            for i in range(len(cluster_index_test)):
                cluster_cell_new[np.where(cluster_cell == tmp_index.index[i])] = i
            cluster_cell = cluster_cell_new
        ...
        for k in range(self.n_components):
            ...
            # 2 gene with one components situlation
            # if(self.observe_data.shape[1] == 2 and self.n_components == 1):
            #     sum_alpha = self.observe_data.mean()
    ...
    def EM(
        ...
        max_round: An integer showing the maximum round. EM will stop immediately when iterate more than max_round times.
        ...
        #void EM_with_1dArr(int model_size, int n_cells, int n_genes, double *ob_data_1d, double *alpha_1d, double *pie, int max_round, double max_pie_tol, double max_loglik_tol, double *res_vec, double **delta)
        self.__lib.EM_with_1dArr(c_int(self.n_components), c_int(self.n_cells), c_int(self.n_genes), ob_data_p, alpha_p, pie_p, c_int(max_round), c_double(max_pie_tol), c_double(max_loglik_tol), c_double(max_alpha_tol), res_vec_p, delta_p)
        self.model['loglik'] = self.__res_vec[0]
        self.model['AIC'] = self.__res_vec[1]
        self.model['BIC'] = self.__res_vec[2]
        if (self.print_log):
