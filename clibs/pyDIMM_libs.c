...
void EM(int model_size, int n_cells, int n_genes, double **ob_data, double **alpha, double *pie, int max_round, double max_pie_tol, double max_loglik_tol, double max_alpha_tol, double *res_vec, double **delta)
{
    ...
    // printf("round[%d] pie_tol[%lf] max_pie_tol[%lf] loglik_tol[%lf] max_loglik_tol[%lf]\n", max_round, pie_tol, max_pie_tol, loglik_tol, max_loglik_tol);
    if ((max_round > 1) && (pie_tol >= max_pie_tol) && (loglik_tol >= max_loglik_tol) && (alpha_tol >= max_alpha_tol))
    {
        // printf("EM again.\n");
        EM(model_size, n_cells, n_genes, ob_data, alpha, pie, max_round-1, max_pie_tol, max_loglik_tol, max_alpha_tol, res_vec, delta);
    }
    return;
}

//The entry for Python calling. Take the 1-D array for ob_data and alpha arguments, reshape them and then call EM function.
void EM_with_1dArr(int model_size, int n_cells, int n_genes, double *ob_data_1d, double *alpha_1d, double *pie, int max_round, double max_pie_tol, double max_loglik_tol, double max_alpha_tol, double *res_vec, double *delta_1d)
{
    ...
    //Call EM.
    EM(model_size, n_cells, n_genes, ob_data, alpha, pie, max_round, max_pie_tol, max_loglik_tol, max_alpha_tol, res_vec, delta);
